...
namespace lionheart.Endpoints.OuraEndpoints
{
        ...
        .WithActionResult<DailyOuraDataDTO>
    {
        ...
        public override async Task<ActionResult<DailyOuraDataDTO>> HandleAsync([FromQuery] DateOnly request, CancellationToken cancellationToken = default)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user is null) { return Unauthorized("User is not recognized or no longer exists."); }

            return this.ToActionResult(await _ouraService.GetDailyOuraInfoAsync(user, request));
        }
    }
}