...
namespace lionheart.Endpoints.MovementEndpoints
{
    ...
    public class GetMovementsEndpoint : EndpointBaseAsync
        .WithRequest<Guid>
        .WithActionResult<List<Movement>>
    {
        ...
        public override async Task<ActionResult<List<Movement>>> HandleAsync([FromRoute] Guid trainingSessionID, CancellationToken cancellationToken = default)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user is null) { return Unauthorized("User is not recognized or no longer exists."); }

            return this.ToActionResult(await _movementService.GetMovementsAsync(user, trainingSessionID));
        }
    }
}