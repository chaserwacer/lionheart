    ...
    for (const injury of activeInjuries) {
      if (injury.injuryEvents) {
        for (const event of injury.injuryEvents) {
          if (
            event.creationTime &&
            new Date(event.creationTime) >= twoWeeksAgo
          ) {
            return { injury, event };
          }
        }
...
>
  ...
  {:else if activeInjuries.length > 0}
    <div>
      <div class="flex items-baseline gap-3">
        <p class="text-7xl font-display font-black text-error leading-none">
      ...
      {#if mostRecentEvent}
        <div class="mt-6 pt-4 border-t-2 border-base-content/10">
          <p class="text-sm font-bold uppercase tracking-wide truncate">
            {mostRecentEvent.injury.name}
          </p>
          <p class="text-xs text-base-content/50 truncate mt-1">
            {mostRecentEvent.event.notes || "Event logged"}
          </p>
        </div>
      {/if}
    </div>
  {:else}
    <div class="py-4">
      <p class="text-7xl font-display font-black text-success leading-none">
        0
      </p>
      <p class="text-xs font-bold uppercase tracking-widest text-success mt-3">
        All clear!
      </p>
    </div>
  {/if}
...
<dialog class="modal" class:modal-open={modalOpen}>
  <div
    class="modal-box max-w-2xl bg-base-100 p-0 overflow-hidden max-h-[90vh] border-2 border-base-content/20"
  >
    <!-- Header -->
    <div class="p-6 pb-4 border-b-2 border-base-content/10">
      <div class="flex items-center justify-between">
        <div>
          ...
          >
            {activeInjuries.length} active {activeInjuries.length === 1
              ? "injury"
              : "injuries"}
          </p>
        </div>
          ...
          {#each activeInjuries as injury}
            <div class="p-5 bg-base-200 border-2 border-base-content/10">
              <div class="flex items-start justify-between mb-3">
                <div>
                  <h4 class="text-xl font-bold uppercase tracking-wide">
                    {injury.name}
                  </h4>
                  <p
                    class="text-xs font-mono uppercase tracking-widest text-base-content/50 mt-1"
                  >
                    Started {formatDate(injury.injuryDate)}
                  </p>
                </div>
                <span
                  class="px-3 py-1 {getPainColor(
                    getMaxPainLevel(injury),
                  ).replace(
                    'badge-',
                    'bg-',
                    ...
                    {#each injury.injuryEvents.slice(0, 3) as event}
                      <div class="flex items-center justify-between text-sm">
                        <span class="text-base-content/70"
                          >{event.notes || "Event"}</span
                        >
                        <span class="text-base-content/50"
                          >{formatDateTime(event.creationTime)}</span
                        >
                      </div>
                    {/each}
                  </div>
